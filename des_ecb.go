...
//TripleEcbDesEncrypt ECB 3DES Encrypt
...
//TripleEcbDesDecrypt ECB 3DES Decrypt
func TripleEcbDesDecrypt(encrypted, key []byte) ([]byte, error) {
	tkey := make([]byte, 24, 24)
	copy(tkey, key)
	k1 := tkey[:8]
	k2 := tkey[8:16]
	k3 := tkey[16:]
	buf1, err := decrypt(encrypted, k3)
	if err != nil {
		return nil, err
	}
